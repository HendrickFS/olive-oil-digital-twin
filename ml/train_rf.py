...
def main():
    parser = argparse.ArgumentParser(description="Train Random Forest Multi-Output Regression Pipeline")
    parser.add_argument('--data', type=str, default='dataset.csv', help='Path to the synthetic CSV dataset')
    parser.add_argument('--model-out', type=str, default='rf_pipeline.pkl', help='Path to save output model')
    ...
    # Use pd.get_dummies for Random Forest
    X = pd.get_dummies(df[features], columns=['cultivar'], drop_first=True)
    Y = df[targets].copy()

    print("Splitting dataset into training (80%) and testing (20%) sets...")
    X_train, X_test, Y_train, Y_test = train_test_split(X, Y, test_size=0.2, random_state=42)

    print("Initializing Random Forest Multi-Output Regressor...")
